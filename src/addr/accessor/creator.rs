...
use crate::addr::access_ctrl::AuthConfig;
...
impl ProxyEP {
    ...
    pub fn from_env(var: &str) -> Option<(Self, Option<AuthConfig>)> {
        std::env::var(var).ok().and_then(|s| Self::parse_url(&s))
    }

    /// 从多个标准环境变量解析代理配置，优先级：HTTPS_PROXY > HTTP_PROXY > ALL_PROXY
    pub fn from_standard_env() -> Option<(Self, Option<AuthConfig>)> {
        // 按优先级顺序检查环境变量
    ...
    fn parse_url(url_str: &str) -> Option<(Self, Option<AuthConfig>)> {
...
mod tests {
    ...
    fn test_parse_url_http() {
        let (config, _auth) = ProxyEP::parse_url("http://127.0.0.1:7890").unwrap();
    ...
    fn test_parse_url_https() {
        let (config, _auth) = ProxyEP::parse_url("https://proxy.example.com:8080").unwrap();
    ...
    fn test_parse_url_socks5() {
        let (config, _auth) = ProxyEP::parse_url("socks5://127.0.0.1:1080").unwrap();
    ...
    fn test_parse_url_with_auth() {
        let (config, auth) = ProxyEP::parse_url("http://user:pass@proxy.example.com:8080").unwrap();
        assert_eq!(
            config.url().as_str(),
    ...
    fn test_parse_url_invalid() {
        let config = ProxyEP::parse_url("invalid-url");
        assert!(config.is_none());
    }

    #[test]
    fn test_parse_url_unsupported_scheme() {
        let config = ProxyEP::parse_url("ftp://proxy.example.com:21");
    ...
    fn test_from_env_var() {
        // 测试单个环境变量解析
        let config = ProxyEP::from_env("TEST_PROXY_VAR");
